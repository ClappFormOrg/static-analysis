...
func acc(rule, file, element string, count int) Acceptance {
	return Acceptance{
		...
		Reviewed: "2026-08-11", Reason: "bounded", Ref: "quality-reports/r.md",
	}
}
...
func TestApplyAcceptancesIgnoresThePrefix(t *testing.T) {
	findings := []Finding{nested("api/internal/a/a.go", "Walk", 10)}
	open, res := ApplyAcceptances(findings, []Acceptance{
		acc(RuleNestedLoop, "internal/a/a.go", "Walk", 1),
	}, "api")
...
func TestBuildIssuesEmitsAResolvedIssueForAFullyAcceptedRule(t *testing.T) {
	res := AcceptanceResult{
		...
		Refs:    map[string][]string{RuleNestedLoop: {"quality-reports/r.md"}},
	}
	...
	for _, want := range []string{"Nothing open", "can be closed", "flat traversal", "quality-reports/r.md"} {
...
func TestWriteTextShowsTheAcceptedColumnAndTheReasons(t *testing.T) {
	...
	out := buf.String()
	for _, want := range []string{"ACCEPTED", "NESTED_LOOP", "58", "flat traversal", "quality-reports/r.md"} {
...
func TestShippedAcceptanceFileIsValid(t *testing.T) {
	...
	for _, a := range accs {
		...
		if !strings.HasPrefix(a.Ref, "quality-reports/") {
...
func TestShippedAcceptanceRefsResolveToAWriteUp(t *testing.T) {
	...
	if _, statErr := os.Stat(refPath(t, "quality-reports/no-such-review.md")); statErr == nil {
